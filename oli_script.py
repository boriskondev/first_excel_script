...
BANKS_DICT = yaml.safe_load(PROJECT_CONFIG.paragraphs[2].text.split("=")[1].replace("\"", ""))
            ...
            for file in element.iterdir():
                if file.suffix == ".xlsx":
                    ...
                    for email in emails_list:
                        ...
                        each_nth = all_selected[(all_selected.index % WEEKLY_EACH_NTH == 0)]
                        nths_df = nths_df.append(each_nth)

                    nths_df.to_excel(weekly_nths, index=False)
                    ...
                    weekly_drawn_df = nths_df.sample(n=WEEKLY_WINNERS + WEEKLY_RESERVES)
                    weekly_drawn_df = weekly_drawn_df.reset_index(drop=True)
                    all_weekly_winners_df = weekly_drawn_df.loc[0:(WEEKLY_WINNERS - 1)]
                    all_weekly_winners_df.insert(0, "Status", "Седмична награда")
                    all_weekly_reserves_df = weekly_drawn_df.loc[WEEKLY_WINNERS:(WEEKLY_WINNERS + WEEKLY_RESERVES - 1)]
                    all_weekly_reserves_df.insert(0, "Status", "Седмична резерва")

                    all_weekly_winners_df.to_excel(writer, sheet_name="Weekly_winners", index=False)
                    all_weekly_reserves_df.to_excel(writer, sheet_name="Weekly_reserves", index=False)

                    writer.save()

                    statistics = append_and_print_statistics(f"Weekly prize entries: {nths_df.shape[0]}", statistics)
                    unique_emails_count = len(nths_df["Email address (Personal)"].unique())
                    statistics = append_and_print_statistics(f"Weekly prize unique emails: {unique_emails_count}\n", statistics)
                    ...
                    writer = pd.ExcelWriter(f"Week_{whole_week}_all_winners_and_reserves.xlsx", engine="xlsxwriter")

                    all_winners = all_weekly_winners_df.append(all_daily_winners_df)
                    all_reserves = all_weekly_reserves_df.append(all_daily_reserves_df)

                    all_winners.to_excel(writer, sheet_name="All_winners", index=False)
                    all_reserves.to_excel(writer, sheet_name="All_reserves", index=False)
                    ...
                    for bank in BANKS_DICT:
                        bank_info_to_write = ""
                        if bank in all_winners["Bank"].unique():
                            winning_bank_df = all_winners[all_winners["Bank"] == bank]
                            bank_info_to_write = f"{BANKS_DICT[bank]}: {winning_bank_df.shape[0]}"
                            bank_name = BANKS_DICT[bank]
                            ...
                            bank_info_to_write = f"{BANKS_DICT[bank]}: 0"
